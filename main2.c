/*
 * @brief Kinect source code for CS/ECE 3992 prototype.
 *
 * This will display the skeleton seen by the Kinect, then perform necessary calculations
 *	to move a foam hand connected to a stepper motor, mimicking the user's hand motions.
 *
 * Adapted from https://github.com/joaquimrocha/Skeltrack/blob/master/examples/test-kinect.c.
 *
 * good luck building this though it took me all day ngl
 *
 * Ella Moss and Dana Escandor
 */
#include <gfreenect.h>
#include <skeltrack.h>
#include <math.h>
...
static void process_hands(SkeltrackJoint *left, SkeltrackJoint *right){
	/*
	if(left && !right){
		printf("Left hand detected! (%d %d %d)\n", left->x, left->y, left->z);
		move_stepper(left->x);
	}
	*/
                  ...
                  gpointer user_data)
{
  ...
  process_hands(left_hand, right_hand);
